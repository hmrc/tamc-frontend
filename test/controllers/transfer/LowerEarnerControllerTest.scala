...
class LowerEarnerControllerTest extends ControllerBaseTest with ControllerViewTestHelper with Injecting {
  ...
  def controller: LowerEarnerController =
    app.injector.instanceOf[LowerEarnerController]
  ...
  "isLowerEarnerAction" should {
    "return bad request" when {
      "an empty form is submitted" in {
        val request = FakeRequest().withMethod("POST").withFormUrlEncodedBody("lowerEarner" -> "")
        val result  = controller.isLowerEarnerAction()(request)
        status(result) mustBe BAD_REQUEST
      }

      "an invalid form is submitted" in {
        val request = FakeRequest().withMethod("POST").withFormUrlEncodedBody("lowerEarner" -> "invalidOption")
        val result  = controller.isLowerEarnerAction()(request)
        status(result).mustBe(BAD_REQUEST)
      }
    }
    ...
    "redirect the user" when {
      "true is selected" in {
        val request = FakeRequest().withMethod("POST").withFormUrlEncodedBody("lowerEarner" -> "true")
        ...
        redirectLocation(result).mustBe(
          Some(controllers.transfer.routes.YourTotalIncomeController.yourTotalIncome().url)
        )
      }

      "false is selected" in {
        val request = FakeRequest().withMethod("POST").withFormUrlEncodedBody("lowerEarner" -> "false")
        ...
        val result = controller.isLowerEarnerAction()(request)
        status(result).mustBe(SEE_OTHER)
        redirectLocation(result) mustBe Some(controllers.transfer.routes.LowerEarnerController.partnerMustApply().url)
      }
    }
  }
}
