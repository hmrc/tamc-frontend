...
class EligibilityCalcControllerTest@Inject()(appConfig: ApplicationConfig) extends ControllerBaseTest {
  ...
  "Check eligibility benefit" should {
    ...
    "Transferor not eligible if transferor income is more than maximum limit" in {
      ...
      val overThreshold = formatter.format(appConfig.MAX_LIMIT() + 100)
    ...
    "show ’recipient is not eligible’ if transferor income=9540  and recipient income>45000" in {
      val formatter = java.text.NumberFormat.getIntegerInstance
      val higherThreshold = formatter.format(appConfig.MAX_LIMIT())
      val overThreshold = formatter.format(appConfig.MAX_LIMIT() + 200)
      val lowerThreshold = formatter.format(appConfig.PERSONAL_ALLOWANCE() + 1)
  ...
  "Check validation message" should {
    ...
    "be displayed if transferor income=9000 (< 9540) and recipient exeeds limit" in {
      ...
      val lowerThreshold = formatter.format(appConfig.PERSONAL_ALLOWANCE() + 1)
      val higherThreshold = formatter.format(appConfig.MAX_LIMIT())
      val request = FakeRequest().withFormUrlEncodedBody("country" -> "england", "transferor-income" -> "9000", "recipient-income" -> (appConfig.MAX_LIMIT() + 1).toString)
      val result = controller.gdsCalculatorAction()(request)
      val document = Jsoup.parse(contentAsString(result))
      document.getElementById("calculator-result").text() shouldBe s"You are not eligible for Marriage Allowance. Your partner’s annual income must be between £$lowerThreshold and £$higherThreshold."
    }
    ...
    "be displayed if transferor income=10000 (9540-11000) and recipient exceeds limit" in {
      ...
      val lowerThreshold = formatter.format(appConfig.PERSONAL_ALLOWANCE() + 1)
      val higherThreshold = formatter.format(appConfig.MAX_LIMIT())
      val request = FakeRequest().withFormUrlEncodedBody("country" -> "england", "transferor-income" -> "10000", "recipient-income" -> (appConfig.MAX_LIMIT() + 1).toString)
      val result = controller.gdsCalculatorAction()(request)
      val document = Jsoup.parse(contentAsString(result))
      document.getElementById("calculator-result").text() shouldBe s"You are not eligible for Marriage Allowance. Your partner’s annual income must be between £$lowerThreshold and £$higherThreshold."
    }
    "be displayed if transferor income is below limit and recipient income=20000" in {
      val formatter = java.text.NumberFormat.getIntegerInstance
      val lowerThreshold = formatter.format(appConfig.PERSONAL_ALLOWANCE())
      val request = FakeRequest().withFormUrlEncodedBody("country" -> "england", "transferor-income" -> (appConfig.PERSONAL_ALLOWANCE() + 1).toString,
